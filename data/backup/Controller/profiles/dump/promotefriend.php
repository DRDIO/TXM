...
else
{
    ...
    $message_promote = ($row['level'] < 2) ? "Would you like to <A HREF='{$friend_url}promotefriend'>promote</A> {$friend_name} to " . strtolower($SITE_USER['friend_level_' . ($row['level'] + 2)]) . ".<BR>" : '';
    $message_demote = ($row['level'] >= 0) ? "Would you like to <A HREF='{$friend_url}demotefriend'>demote</A> {$friend_name} to " . strtolower($SITE_USER['friend_level_' . ($row['level'])]) . ".<BR>"     :
    ...
    if($row['level'] == 3) message_die(GENERAL_ERROR, "Your friendship with <A HREF='{$friend_url}'>{$friend_name}</A> is already fully promoted.<BR>{$message_promote}{$message_demote}<BR><A HREF='{$friend_url}'>Return to {$friend_name}'s profile.</A><BR>");
    
    $sql = "UPDATE " . FRIENDS_TABLE . " SET level=level+1 WHERE user_id_1={$SITE_USER['user_id']} AND user_id_2={$friend_id}";
    if(!($result = $db->sql_query($sql))) message_die(GENERAL_ERROR, 'Unable to retrieve members information.', $sql);
    
    message_die(GENERAL_MESSAGE, "You have promoted your friendship with <A HREF='{$friend_url}'>{$friend_name}</A> to " . strtolower($SITE_USER['friend_level_' . ($row['level'] + 1)]) . ".<BR>{$message_promote}{$message_demote}<BR><A HREF='{$friend_url}'>Return to {$friend_name}'s profile.</A><BR>");
}

...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
    ...
    $R->pageTitle = 'game "' . $row['title'] . '" by ' . $row['nick_name'];
/*
// Grab Ten Best of Author's Other games
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
    $sql = "
        SELECT
            id,
    ...
    if($result === FALSE)
    {
        message_die(GENERAL_ERROR, "We were unable to retrieve other games by author.", $sql);
    }
    else
    {
        $count = 0;
        while($more = $db->sql_fetchrow($result))
        {
            ...
            $more["avatar"] = (file_exists($SITE["level"] . "media/games/screenshots/" . $more["id"] . ".gif") === TRUE ?
                "games/screenshots/" . $more["id"] : "assets/games-icon");
            $template->assign_block_vars("games_more", $more);
        }
        ...
        if($count < 5)
        {
            for($count; $count < 5; $count)
            {
                $more = array();
                $more["id"] = ".";
                $more["title"] = "Upload Your Own game";
                $more["synopsis"] = "Uploading your Flash game is quick and easy, so get started!";
                $more["color"] = $count++ % 2 === 0 ? "blu-alt" : "";
                $more["title_link"] = "upload";
                $more["avatar"] = "assets/games-icon";
                $template->assign_block_vars("games_more", $more);
            }
        }
    }
...
// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
    ...
    if($result === FALSE)
    {
        message_die(GENERAL_ERROR, "We were unable to retrieve random games.", $sql);
    }
    else
    {
        $count = 0;
        while($random = $db->sql_fetchrow($result))
        {
            ...
            $random["avatar"] = (file_exists($SITE["level"] . "media/games/screenshots/" . $random["id"] . ".gif") === TRUE ?
                "games/screenshots/" . $random["id"] : "assets/games-icon");

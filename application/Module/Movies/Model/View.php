...
class Movies_Model_View extends Main_Model_Abstract
{
    public static function getInfo($movieId)
    {
        $db     = self::getDb();
        $select = $db->select()
    ...
    public static function updateViewCount($movieId)
    {
        $db       = self::getDb();
        $affected = $db->update('movies', array(
            'view_offset' => new Zend_Db_Expr('view_offset + 1')
        ), $db->quoteIdentifier('id') . ' = ' . $db->quote($movieId));

...
require_once 'Rackspace/Object.php';
...
class Rackspace_Container
{
     ...
     *
     * @param obj $cfs_auth Rackspace_Authentication instance
     ...
     * @throws Rackspace_Exception invalid Container name
     */
    function __construct(&$cfs_auth, &$cfs_http, $name, $count=0,
        $bytes=0, $docdn=True)
    {
        if (strlen($name) > Rackspace_Connection::MAX_CONTAINER_NAME_LEN) {
            throw new Rackspace_Exception("Container name exceeds "
     ...
     * @throws Rackspace_Exception CDN functionality not returned during auth
     * @throws Rackspace_Exception if auth token is not valid/expired
     ...
     * @throws Rackspace_Exception if CDN Is not enabled on this connection
     * @throws Rackspace_Exception if the response expected is not returned
     */
     ...
     * @return obj Rackspace_Object instance
     */
    function create_object($obj_name=NULL)
    {
        return new Rackspace_Object($this, $obj_name);
    }
    ...
    {
        return new Rackspace_Object($this, $obj_name, True);
    }
    ...
    function getObjectList($limit=0, $marker=NULL, $prefix=NULL, $path=NULL)
    {
        ...
        foreach ($obj_array as $obj) {
            $tmp = new Rackspace_Object($this, $obj["name"], False, False);
     ...
     * @throws Rackspace_Exception invalid Object/Container name
     * @throws Rackspace_Exception remote Object does not exist
     * @throws Rackspace_Exception unexpected response
     */
    function copy_object_to($obj,$container_target,$dest_obj_name=NULL,$metadata=NULL,$headers=NULL)
    {
        $obj_name = NULL;
        if (is_object($obj)) {
            if (get_class($obj) == "Rackspace_Object") {
                $obj_name = $obj->name;
            }
        }
        if (is_string($obj)) {
        ...
        if (!$obj_name) {
            throw new Rackspace_Exception("Object name not set.");
        }
        ...
        if (is_object($container_target)) {
            if (get_class($container_target) == "Rackspace_Container") {
        ...
        if (!$container_name_target) {
            throw new Rackspace_Exception("Container name target not set.");
        }
        ...
        if ($status == 404) {
            ...
            throw new Rackspace_Exception($m);
        }
        if ($status < 200 || $status > 299) {
            throw new Rackspace_Exception(
                "Invalid response (".$status."): ".$this->cfs_http->get_error());
    ...
    function copy_object_from($obj,$container_source,$dest_obj_name=NULL,$metadata=NULL,$headers=NULL)
    {
        ...
        if (is_object($container_source)) {
            if (get_class($container_source) == "Rackspace_Container") {
        ...
        if (!$container_name_source) {
            throw new Rackspace_Exception("Container name source not set.");
        }
     ...
     * @throws Rackspace_Exception invalid Object name
     * @throws Rackspace_Exception remote Object does not exist
     * @throws Rackspace_Exception unexpected response
     */
    ...
    function create_paths($path_name)
    {
        ...
        foreach ($elements as $idx => $val) {
            ...
            $obj = new Rackspace_Object($this, $build_path);

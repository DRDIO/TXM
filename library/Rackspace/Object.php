...
class Rackspace_Object
{
    ...
    const MAX_OBJECT_SIZE = 5368709121;
     ...
     * Class constructor
     *
     * @param obj $container Rackspace_Container instance
    ...
    function __construct(&$container, $name, $force_exists=False, $dohead=True)
    {
        ...
        if (strlen($name) > self::MAX_OBJECT_NAME_LEN) {
            throw new Rackspace_Exception("Object name exceeds "
        ...
        if ($dohead) {
            if (!$this->_initialize() && $force_exists) {
                throw new Rackspace_Exception("No such object '".$name."'");
            }
        }
    }
    ...
    function _guess_content_type($handle) {
        ...
        if (!$this->content_type) {
            throw new Rackspace_Exception("Required Content-Type not set");
        }
        return True;
    }
    ...
    function read($hdrs=array())
    {
        ...
        if (($status < 200) || ($status > 299
            ...
            throw new Rackspace_Exception("Invalid response (".$status."): "
     ...
     * @throws Rackspace_Exception missing required parameters
     * @throws Rackspace_Exception if no Content-Type was/could be set
     * @throws Rackspace_Exception $verify is set and checksums unequal
     * @throws Rackspace_Exception unexpected response
     */
    function write($data=NULL, $bytes=0, $verify=True)
    {
        if (!$data && !is_string($data)) {
            throw new Rackspace_Exception("Missing data source.");
        }
        if ($bytes > self::MAX_OBJECT_SIZE) {
            throw new Rackspace_Exception("Bytes exceeds maximum object size.");
        }
        if ($verify) {
        ...
        if (!is_resource($data)) {
            ...
            if ($this->content_length > self::MAX_OBJECT_SIZE) {
                throw new Rackspace_Exception("Data exceeds maximum object size");
            }
        ...
        if ($status == 412) {
            ...
            throw new Rackspace_Exception("Missing Content-Type header");
        }
        if ($status == 422) {
            if ($close_fh) { fclose($fp); }
     ...
     * @throws Rackspace_Exception $verify is set and checksums unequal
     * @throws Rackspace_Exception unexpected response
     * @throws Rackspace_Exception error opening file
     */
    function load_from_filename($filename, $verify=True)
    {
        $fp = @fopen($filename, "r");
        if (!$fp) {
            throw new Rackspace_Exception("Could not open file for reading: ".$filename);
        }
        ...
        if ($size > self::MAX_OBJECT_SIZE) {
            throw new Rackspace_Exception("File size exceeds maximum object size.");
        }
    ...
    function save_to_filename($filename)
    {
        ...
        if (!$fp) {
            throw new Rackspace_Exception("Could not open file for writing: ".$filename);
        }

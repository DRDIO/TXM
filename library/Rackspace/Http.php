 ...
 * @package php-cloudfiles-http
 */
class Rackspace_Http
{
    const PHP_CF_VERSION = "1.7.10";
    const USER_AGENT = "PHP-CloudFiles/1.7.10";
    ...
    const CONTAINER_BYTES_USED = "X-Container-Bytes-Used";
    const MANIFEST_HEADER = "X-Object-Manifest";
    const METADATA_HEADER_PREFIX = "X-Object-Meta-";
    const CONTENT_HEADER_PREFIX = "Content-";
    const ACCESS_CONTROL_HEADER_PREFIX = "Access-Control-";
    const ORIGIN_HEADER = "Origin";
    const CDN_URI = "X-CDN-URI";
    const CDN_SSL_URI = "X-CDN-SSL-URI";
    const CDN_STREAMING_URI = "X-CDN-Streaming-URI";
    ...
    function ssl_use_cabundle($path=NULL)
    {
        ...
        if (!file_exists($this->cabundle_path)) {
            throw new Rackspace_Exception("Could not use CA bundle: "
                . $this->cabundle_path);
        }
        return;
    }
    ...
    function authenticate($user, $pass, $acct=NULL, $host=NULL)
    {
        ...
        if (isset($acct)){
            $headers = array(
                sprintf("%s: %s", self::AUTH_USER_HEADER_LEGACY, $user),
                sprintf("%s: %s", self::AUTH_KEY_HEADER_LEGACY, $pass),
                );
            $path[] = $host;
            ...
            $headers = array(
                sprintf("%s: %s", self::AUTH_USER_HEADER, $user),
                sprintf("%s: %s", self::AUTH_KEY_HEADER, $pass),
                );
	    $path[] = $host;
        }
	$path[] = "v1.0";
        ...
        curl_setopt($curl_ch, CURLOPT_USERAGENT, self::USER_AGENT);
    ...
    {
        if(!$path)
            throw new Rackspace_Exception("Path not set");
        $url_path = $this->_make_path("CDN", NULL, $path);
        if($email)
        {
            $hdrs = array(self::CDN_EMAIL => $email);
    ...
    {
        if ($container_name == "")
            throw new Rackspace_Exception("Container name not set.");
        ...
        $hdrs = array(
            ...
            self::CDN_TTL => $ttl,
            self::CDN_LOG_RETENTION => $cdn_log_retention ?  "True" : "False",
    ...
    function remove_cdn_container($container_name)
    {
        ...
        $url_path = $this->_make_path("CDN", $container_name);
        $hdrs = array(self::CDN_ENABLED => "False");
        $return_code = $this->_send_request("DEL_POST",$url_path,$hdrs,"POST");
        if ($return_code == 401) {
            $this->error_str = "Unauthorized";
    ...
    #
    function getObjectList($cname,$limit=0,$marker=NULL,$prefix=NULL,$path=NULL)
    {
        if (!$cname) {
            $this->error_str = "Container name not set.";
            return array(0, $this->error_str, array());
    ...
    {
        if (!is_object($obj) || get_class($obj) != "Rackspace_Object") {
            throw new Rackspace_Exception(
                "Method argument is not a valid Rackspace_Object.");
        }
    ...
    private function _header_cb($ch, $header)
    {
        ...
        case strtolower(self::CDN_ENABLED):
            $this->_cdn_enabled = strtolower($value) == "true";
            break;
        case strtolower(self::CDN_URI):
            $this->_cdn_uri = $value;
            break;
        case strtolower(self::CDN_SSL_URI):
            $this->_cdn_ssl_uri = $value;
            break;
        case strtolower(self::CDN_STREAMING_URI):
            $this->_cdn_streaming_uri = $value;
            break;
        case strtolower(self::CDN_TTL):
            $this->_cdn_ttl = $value;
            break;
        case strtolower(self::MANIFEST_HEADER):
            $this->_obj_manifest = $value;
            break;
        case strtolower(self::CDN_LOG_RETENTION):
            $this->_cdn_log_retention = strtolower($value) == "true";
            break;
        case strtolower(self::CDN_ACL_USER_AGENT):
            $this->_cdn_acl_user_agent = $value;
            break;
        case strtolower(self::CDN_ACL_REFERRER):
            $this->_cdn_acl_referrer = $value;
            break;
        case strtolower(self::ACCOUNT_CONTAINER_COUNT):
            $this->_account_container_count = (float)$value+0;
            break;
        case strtolower(self::ACCOUNT_BYTES_USED):
            $this->_account_bytes_used = (float)$value+0;
            break;
        case strtolower(self::CONTAINER_OBJ_COUNT):
            $this->_container_object_count = (float)$value+0;
            break;
        case strtolower(self::CONTAINER_BYTES_USED):
            $this->_container_bytes_used = (float)$value+0;
            break;
        case strtolower(self::ETAG_HEADER):
            $this->_obj_etag = $value;
            break;
        case strtolower(self::LAST_MODIFIED_HEADER):
            $this->_obj_last_modified = $value;
            break;
        case strtolower(self::CONTENT_TYPE_HEADER):
            $this->_obj_content_type = $value;
            break;
        case strtolower(self::CONTENT_LENGTH_HEADER):
            $this->_obj_content_length = (float)$value+0;
            break;
        case strtolower(self::ORIGIN_HEADER):
            $this->_obj_headers[self::ORIGIN_HEADER] = $value;
            break;
        default:
            if (strncasecmp($name, self::METADATA_HEADER_PREFIX, strlen(self::METADATA_HEADER_PREFIX)) == 0) {
                $name = substr($name, strlen(self::METADATA_HEADER_PREFIX));
                $this->_obj_metadata[$name] = $value;
            }
            elseif ((strncasecmp($name, self::CONTENT_HEADER_PREFIX, strlen(self::CONTENT_HEADER_PREFIX)) == 0) ||
                    (strncasecmp($name, self::ACCESS_CONTROL_HEADER_PREFIX, strlen(self::ACCESS_CONTROL_HEADER_PREFIX)) == 0)) {
    ...
    private function _auth_hdr_cb($ch, $header)
    {
        ...
        if (stripos($header, self::STORAGE_URL) === 0) {
            $this->storage_url = trim(substr($header, strlen(self::STORAGE_URL)+1));
        }
        if (stripos($header, self::CDNM_URL) === 0) {
            $this->cdnm_url = trim(substr($header, strlen(self::CDNM_URL)+1));
        }
        if (stripos($header, self::AUTH_TOKEN) === 0) {
            $this->auth_token = trim(substr($header, strlen(self::AUTH_TOKEN)+1));
        }
        if (stripos($header, self::AUTH_TOKEN_LEGACY) === 0) {
            $this->auth_token = trim(substr($header,strlen(self::AUTH_TOKEN_LEGACY)+1));
        }
    ...
    private function _make_headers($hdrs=NULL)
    {
        ...
        if (is_array($hdrs)) {
            foreach ($hdrs as $h => $v) {
                ...
                if (strncasecmp($h, self::AUTH_TOKEN, strlen(self::AUTH_TOKEN)) === 0) {
                    $has_stoken = True;
                }
                if (strncasecmp($h, self::USER_AGENT_HEADER, strlen(self::USER_AGENT_HEADER)) === 0) {
                    $has_uagent = True;
                }
        ...
        if (!$has_stoken) {
            $new_headers[] = self::AUTH_TOKEN . ": " . $this->auth_token;
        }
        if (!$has_uagent) {
            $new_headers[] = self::USER_AGENT_HEADER . ": " . self::USER_AGENT;
        }
        return $new_headers;
    }
    ...
    private function _headers(&$obj)
    {
        ...
        if ($obj->manifest)
            $hdrs[self::MANIFEST_HEADER] = $obj->manifest;
    ...
    private function _process_headers($metadata=null, $headers=null)
    {
        $rules = array(
            array(
                'prefix' => self::METADATA_HEADER_PREFIX,
            ),
            array(
                'prefix' => '',
                'filter' => array( # key order is important, first match decides
                    self::CONTENT_TYPE_HEADER          => false,
                    self::CONTENT_LENGTH_HEADER        => false,
                    self::CONTENT_HEADER_PREFIX        => true,
                    self::ACCESS_CONTROL_HEADER_PREFIX => true,
                    self::ORIGIN_HEADER                => true,
                ),
            ),
        );
        ...
        for ($argi = 0; $argi < $argc; $argi++) {
            ...
            foreach ($argv[$argi] as $k => $v) {
                $k = trim($k);
                $v = trim($v);
                if (strpos($k, ":") !== False) throw new Rackspace_Exception(
                ...
                if (array_key_exists('filter', $rule)) {
                    ...
                    if (!$result) throw new Rackspace_Exception(sprintf(
                        "Header name %s is not allowed", $k));
                }

                $k = $rule['prefix'] . $k;
                if (strlen($k) > self::MAX_HEADER_NAME_LEN || strlen($v) > self::MAX_HEADER_VALUE_LEN)
                    throw new Rackspace_Exception(sprintf(

...
require_once 'Rackspace/Container.php';
 ...
 * # validated Rackspace_Authentication instance.
 * #
 * $conn = new Rackspace_Connection($auth);
 *
...
class Rackspace_Connection
{
    ...
    /**
     * Pass in a previously authenticated Rackspace_Authentication instance.
     *
     * Example:
     * <code>
     * # Create the authentication instance
     ...
     * # like this.
     *
     * $conn = new Rackspace_Connection($auth, $servicenet=True);
     *
     * </code>
     *
     ...
     *
     * @param obj $cfs_auth previously authenticated Rackspace_Authentication instance
     * @param boolean $servicenet enable/disable access via Rackspace servicenet.
     * @throws Rackspace_Exception not authenticated
     */
    function __construct($cfs_auth, $servicenet=False)
    {
        ...
        if (!$this->cfs_auth->authenticated()) {
            ...
            $e .= "Rackspace_Authentication instance.";
            throw new Rackspace_Exception($e);
        }
     ...
     * #
     * $conn = new Rackspace_Authentication($auth);
     *
     ...
     * @throws Rackspace_Exception invalid name
     * @throws Rackspace_Exception unexpected response
     */
    function create_container($container_name=NULL)
    {
        ...
        if (strpos($container_name, "/") !== False) {
            ...
            throw new Rackspace_Exception($r);
        }
        if (strlen($container_name) > self::MAX_CONTAINER_NAME_LEN) {
        ...
        if (!$return_code) {
            throw new Rackspace_Exception("Invalid response ("
        ...
        return new Rackspace_Container($this->cfs_auth, $this->cfs_http, $container_name);
    }
     ...
     * @throws Rackspace_Exception missing proper argument
     * @throws Rackspace_Exception invalid response
     * @throws Rackspace_Exception container not empty
     * @throws Rackspace_Exception remote container does not exist
     */
    function delete_container($container=NULL)
    {
        ...
        if (is_object($container)) {
            if (get_class($container) == "Rackspace_Container") {
        ...
        if ($container_name != "0" and !isset($container_name))
            throw new Rackspace_Exception("Must specify container object or name.");

        $return_code = $this->cfs_http->delete_container($container_name);

        if (!$return_code) {
            throw new Rackspace_Exception("Failed to obtain http response");
        }
        #if ($status == 401 && $this->_re_auth()) {
     ...
     * @return container Rackspace_Container instance
     * @throws Rackspace_Exception thrown if no remote Container
     * @throws Rackspace_Exception unexpected response
     */
    function get_container($container_name=NULL)
    {
        ...
        #}
        if ($status == 404) {
            throw new Rackspace_Exception("Container not found.");
        }
        if ($status < 200 || $status > 299) {
            throw new Rackspace_Exception(
                "Invalid response: ".$this->cfs_http->get_error());
        }
        return new Rackspace_Container($this->cfs_auth, $this->cfs_http,
     ...
     *
     * Return an array of Rackspace_Container instances on the account.  The instances
     ...
     * </code>
     *
     * @return array An array of Rackspace_Container instances
     * @throws Rackspace_Exception unexpected response
     */
    function get_containers($limit=0, $marker=NULL)
    {
        ...
        foreach ($container_info as $name => $info) {
            $containers[] = new Rackspace_Container($this->cfs_auth, $this->cfs_http,
     ...
     * }
     *
     * $conn = new Rackspace_Connection($auth_obj);
    ...
    #{
    #    $new_auth = new Rackspace_Authentication(
